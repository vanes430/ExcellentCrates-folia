...
public class RewardSortingDialog extends Dialog<Crate> {
    ...
    public WrappedDialog create(@NotNull Player player, @NotNull Crate crate) {
        ...
        for (SortMode mode : SortMode.values()) {
            buttons.add(
                DialogButtons.action(BUTTON_MODE.replace(str -> str.replace("%s", SORT_MODE_LOCALE.getLocalized(mode))))
                    .action(DialogActions.customClick("ok", NightNbtHolder.builder().put(JSON_MODE, LowerCase.INTERNAL.apply(mode.name())).build()))
                    .build()
            );
        }
