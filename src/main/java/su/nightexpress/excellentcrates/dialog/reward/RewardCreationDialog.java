...
public class RewardCreationDialog extends Dialog<RewardCreationDialog.Data> {
    ...
    public WrappedDialog create(@NotNull Player player, @NotNull Data data) {
        ...
        boolean isCustom = ItemHelper.isCustom(itemStack);
        ...
        for (RewardType type : RewardType.values()) {
            ...
            buttons.add(DialogButtons.action(locale).action(DialogActions.customClick("ok", NightNbtHolder.builder().put(JSON_TYPE, type.name()).build())).build());
        }
        ...
        return Dialogs.create(builder -> {
            ...
            builder.base(DialogBases.builder(TITLE)
                .body(
                    DialogBodies.item(itemStack).build(),
            ...
            builder.type(DialogTypes.multiAction(buttons).exitAction(DialogButtons.action("Back").action(DialogActions.customClick("back")).build()).build());
            ...
            builder.handleResponse("ok", (viewer, identifier, nbtHolder) -> {
                ...
                ItemProvider provider = ItemHelper.adapt(itemStack, allowCustoms);
                Reward reward = RewardFactory.wizardCreation(this.plugin, crate, itemStack, type, provider);
                crate.addReward(reward);
                crate.saveRewards();
                viewer.getPlayer().closeInventory();
                Dialog.handleCallback(viewer.getPlayer());
            });
        });
    }
}
